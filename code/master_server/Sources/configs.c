...
void read_values_cfg_master(void){
	...
	if(will_use_tls){
		...
		snprintf(auth_cert_file_path,sizeof(auth_cert_file_path),"%s",(char*)&curr_line_buff[strlen("master_auth_cert_file_path: ")]);
	       	clean_buff();
		skip_config_comments(cfg_fp);
		if(!(fgets(curr_line_buff,CONFIG_READ_LINE_BUFF_SIZE,cfg_fp))){
	        ...
	        curr_line_buff[strlen(curr_line_buff)-1]=0;
		snprintf(host_cert_file_path,sizeof(host_cert_file_path),"%s",(char*)&curr_line_buff[strlen("master_host_cert_file_path: ")]);
	       	clean_buff();
		skip_config_comments(cfg_fp);
