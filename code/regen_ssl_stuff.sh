...
cre8_cert_module_name_files(){
	for((i=0; i<$total; i++)); 
	do
		touch "$module_name_file"
		"${directories_to_work_on[$i]}"
		cp "${name_string_for_authority_stuff}_${host_string_in_file_name}.conf" "${complete_cert_directories_to_work_on[$i]}"
	done
}
...
export_host_certs_to_dirs(){
	...
	do
		...
		openssl req -new -key "${name_string_for_module}.key" -out "${name_string_for_module}.csr" -config "${name_string_for_module}.conf"


		openssl x509 -req -in "${name_string_for_module}.csr" -CA "${name_string_for_authority_stuff}.crt" -CAkey "${name_string_for_authority_stuff}.key" -CAcreateserial -out "${name_string_for_module}.crt" -days 825 -sha256 -extensions v3_req -extfile "${name_string_for_module}.conf"
		
		curr_tg_dir="${complete_cert_directories_to_work_on[$i]}"
		mv * "${complete_cert_directories_to_work_on[$i]}"
	done
}
